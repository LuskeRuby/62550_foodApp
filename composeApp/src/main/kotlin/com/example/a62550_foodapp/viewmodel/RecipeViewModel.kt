    ...
    suspend fun getRecipePrice(recipeId: Int): Float {
        return try {
            ...
            for (recipeItem in recipeItems) {
                val itemPrices =
                    itemWeeklyPriceDao.getItemSizesAndMinPrices(recipeItem.itemGroupId)
                    ...
                    recipeItems.map { recipeItem ->
                        ...
                        pricesFlow.map { itemPrices ->
                            ...
                            for (item in itemPrices) {
                                if (item.size > 0f) {
                                    val packagesNeeded =
                                        kotlin.math.ceil(recipeItem.quantity / item.size)
                                            .toInt()
                                    val totalCost = packagesNeeded * item.price
                                    cheapestCost = minOf(cheapestCost, totalCost.toFloat())
                                }
            ...
            for (recipeItem in recipeItems) {
                ...
                for (item in itemPrices) {
                    if (item.size > 0) {
                        ...
                        val packagesNeeded =
                            kotlin.math.ceil(scaledQuantity / item.size).toInt()
                        val totalCost = packagesNeeded * item.price
                        cheapestCost = minOf(cheapestCost, totalCost.toFloat())
                    }
                }
